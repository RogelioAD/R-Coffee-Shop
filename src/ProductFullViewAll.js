...
function ProductFullViewAll() {
  ...
  useEffect(() => {
    ...
    async function fetch() {
      await getProductId(params.productID).then((product) =>
        setProducts(product)
      );
  ...
  return (
    <div className="fullmaincardpaddingx">
      <Card
        key={id}
        className="bordercard fullcard-design font d-inline-block align-top"
      >
        <Card.Body className="padding d-flex flex-column justify-content-center align-items-center">
          ...
          >
            <Link to={`/${id}/edit-all`} className="edit btn btn-primary ms-1">
              Edit
            </Link>
            <Button
              variant="danger"
...
export default ProductFullViewAll;
