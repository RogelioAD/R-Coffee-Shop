...
function ProductsViewAll() {
    ...
    let { getProductAll, deleteProduct, getSearch, getAsc, getDesc, getAlphabeticalAsc, getAlphabeticalDesc, getRange24, getRange45, getRange5 } = useContext(ProductContext)
                ...
                <Card key={id} className="bordercard card-design font d-inline-block align-top">
                    <Card.Body className="padding d-flex flex-column justify-content-center align-items-center">
                        ...
                        <div style={{ transform: 'scale(0.8)' }} className="d-flex align-items-center">
                            <Link to={`/${id}/full-view-all`} className="view btn btn-secondary ms-1">View</Link>
                            <Link to={`/${id}/edit-all`} className="edit btn btn-primary ms-1">Edit</Link>
                            ...
                            <Row className='mx-auto'>
                                ...
                                <Col md={2}>
                                    <Dropdown className="font">
                                        <Dropdown.Toggle className="sort" variant="success" id="dropdown-basic">
                                            Sort
                                        </Dropdown.Toggle>
                                        <Dropdown.Menu>
                                            <Dropdown.Item onClick={alphaA}>A-Z</Dropdown.Item>
                                            <Dropdown.Item onClick={alphaD}>Z-A</Dropdown.Item>
                                            <Dropdown.Item onClick={desc}>Price High-Low</Dropdown.Item>
                                            <Dropdown.Item onClick={asc}>Price Low-High</Dropdown.Item>
                                        </Dropdown.Menu>
                                    </Dropdown>
                                    <Form>
                                ...
                                <Col md={6}>
                                    {['checkbox'].map((type) => (
                                        <div key={`inline-${type}`} className="filterfont mt-2 d-flex justify-content-end">
                                            <Form.Check
                                                inline
                                                label="$2-$4"
                                                name="group1"
